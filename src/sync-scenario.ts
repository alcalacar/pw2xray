...
import { graphql, importFeature, addTestsToTestPlan, removeTestsFromTestPlan, getTestByKey } from "./xray";
...
function extractFeatureName(lines: string[]): string {
	...
	if (!line) throw new Error("No se encontró una línea 'Feature:' en el archivo.");
...
function extractBackground(lines: string[]): string | null {
...
interface ScenarioBlock {
	scenarioLineIdx: number;
	block: string;
	stepsText: string;
}

function findScenarioBlock(lines: string[], scenarioName: string): ScenarioBlock {
...
export async function syncScenario(token: string, options: SyncScenarioOptions): Promise<SyncScenarioResult> {
	const { featurePath, projectKey, scenarios, testPlanIssueId } = options;
	const scenarioNames = Object.keys(scenarios);
	if (scenarioNames.length === 0) throw new Error("El mapa de escenarios está vacío.");
	...
	const tmpFile = path.join(
		...
		`_tmp_sync_${Date.now()}_${Math.random().toString(36).slice(2)}.feature`
	);
	const tmpContent =
		[`Feature: ${featureName}`, "", background, background ? "" : null, ...targets.map((t) => t.block)]
			.filter((l): l is string => l !== null)
			.join("\n\n") + "\n";
	fs.writeFileSync(tmpFile, tmpContent, "utf8");

	try {
		const result = await importFeature(token, projectKey, tmpFile);
		...
		if (created.length !== targets.length) {
			throw new Error(
				`Se esperaban ${targets.length} Tests creados/actualizados, llegaron ${created.length}. ` +
		...
		const data = await graphql<{ getTests: { results: { issueId: string; jira: { key: string }; gherkin?: string }[] } }>(
			token,
			`query($jql: String!) {
		...
		const matches = new Map<string, SyncScenarioMatch>();
		for (const target of targets) {
			const candidates = remoteTests.filter((rt) => (rt.gherkin || "").trim() === target.stepsText.trim());
			if (candidates.length !== 1) {
				throw new Error(
					`Verificación falló para "${target.name}": ${candidates.length} Test(s) remoto(s) coinciden ` +
		...
		for (const { atIndex, text } of insertions) newLines.splice(atIndex, 0, text);
		fs.writeFileSync(featurePath, newLines.join("\n"), "utf8");
		...
		if (testPlanIssueId) {
			for (const target of targets) {
				...
				if (!deprecatesKey) {
					await addTestsToTestPlan(token, testPlanIssueId, [newIssueId]);
					continue;
				}
				const oldTest = await getTestByKey(token, deprecatesKey);
				if (!oldTest) {
					console.warn(`[WARN] No se encontró ${deprecatesKey} en Xray -- no se actualiza el Test Plan.`);
					continue;
				}
				await addTestsToTestPlan(token, testPlanIssueId, [newIssueId]);
				await removeTestsFromTestPlan(token, testPlanIssueId, [oldTest.issueId]);
			}
		}
		...
		let preconditionWarning: string | undefined;
		if (background) {
			const preconditionsData = await graphql<{
				getTests: { results: { jira: { key: string }; preconditions: { results: { issueId: string; jira: { key: string; summary: string } }[] } }[] };
			}>(
				token,
				`query($jql: String!) {
					getTests(jql: $jql, limit: 50) {
			...
			const preconditionsPorId = new Map<string, { key: string; summary: string }>();
			for (const t of preconditionsData.getTests.results) {
				for (const p of t.preconditions.results) preconditionsPorId.set(p.issueId, p.jira);
			}
